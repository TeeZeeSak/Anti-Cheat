...
stock logCheater(id, const input[], any: ...){
	new _iCurrentTime = get_systime(0);
	new deltaTime = _iCurrentTime - g_iLastDetect[id]
	if(deltaTime >= 5){ //Only log if last log was atleast 5 seconds ago	
		g_iDetections[id]++;
		static msg[ 256 ] 
		vformat(msg, 255, input, 3) 
		log_to_file("anticheat-TestPhase.log", msg);
		
		g_iLastDetect[id] = get_systime(0)
	}
}
...
public fw_CmdStart ( id , uc_handle ) {
	...
	if(g_iCurrFPS[id] < 30 && g_iCmdRate[id] > g_iCurrFPS[id] + 85 && g_iCurrFPS[id] > 0 && pev ( id , pev_maxspeed ) > 200){
		new name [32] , steamid [32];
		get_user_name ( id , name , charsmax(name) );
		get_user_authid ( id , steamid , charsmax(steamid) );
		//g_bBanned [id] = true;
		logCheater (id, "[Anti-Cheat] Player %s(%s) [%i] is using speedhack! %i | %i" , name , steamid, g_iDetections[id], g_iCurrFPS[id], g_iCmdRate[id]);
	...
	if(g_iTotalKnife[id] >= 30){
		...
		if(flRatio >= 30 && flRatio <= 45){
			...
			get_user_name ( id , name , charsmax(name) );
			get_user_authid ( id , steamid , charsmax(steamid) );
			logCheater(id, "[Anti-Cheat] Player %s(%s) [%i] is using knifebot!", name , steamid, g_iDetections[id]);
			g_iTotalKnife[id] = 0;
			g_iKnifeTime[id] = 0;
		}
	}
...
public fw_PlayerPostThink ( id , uc_handle ) {
	...
	if ( g_flForwardMove[id] > flMaxSpeed || g_flSideMove[id] > flMaxSpeed || g_flForwardMove[id] < -flMaxSpeed || g_flSideMove[id] < -flMaxSpeed ) {
		...
		get_user_authid ( id , steamid , charsmax(steamid) );
		//g_bBanned [id] = true;
		logCheater(id, "[Anti-Cheat] Player %s(%s) [%i] is using strafehack - limits. Forward: %.1f Side: %.1f" , name , steamid , g_iDetections[id], g_flForwardMove[id] , g_flSideMove[id]);
	...
	if ( g_iMove [id][LEFT] > 2 && !g_bStrafeMod [id] && button & IN_MOVELEFT && oldbuttons & IN_MOVELEFT && !(button & IN_FORWARD) && !(button & IN_BACK) ) {
		if ( g_flForwardMove[id] != 0.0 ) {
			...
			logCheater(id, "[Anti-Cheat] Player %s(%s) [%i] is using strafehack - Buttons" , name , steamid, g_iDetections[id] );
		}
	}
	...
	if ( !g_bStrafeMod[id] && g_iMove [id][LEFT] > 3 && g_iMove [id][UP] > 2 && button & IN_MOVELEFT && button & IN_FORWARD && !(button & IN_BACK) && !(button & IN_MOVERIGHT) ) {
		...
		if ( g_flSideMove [id] > -flMaxSpeed+2 || g_flSideMove [id] < -flMaxSpeed-2 || g_flForwardMove [id] > flMaxSpeed+2 || g_flForwardMove [id] < flMaxSpeed-2 ) {
			...
			logCheater(id, "[Anti-Cheat] Player %s(%s) [%i] is using strafehelper - Button/vector difference" , name , steamid, g_iDetections[id] );
		}
	}
...
public fw_PlayerPreThink ( id ) {
	...
	if ( g_iTotalBhop [id][RATIO] >= 60 )
	{
		...
		if ( flRatio >= 99.0 )
		{
			...
			logCheater(id, "[Anti-Cheat] Player %s(%s) [%i] is using bhop hack - Ratio. fLRatio: %.1f", szName, szSteamId, g_iDetections[id], flRatio);
	...
	if ( g_iPerfectBhop [id] [FOG1] >= MAXPERFECT )
	{
		...
		get_user_name ( id , szName , charsmax(szName) );
		get_user_authid ( id , szSteamId , charsmax(szSteamId) );
		logCheater(id, "[Anti-Cheat] Player %s(%s) [%i] is using bhop hack - MaxPerfect. g_iPerfectBhop: %i/%i", szName, szSteamId, g_iDetections[id], g_iPerfectBhop[id][FOG1], MAXPERFECT);
		g_iPerfectBhop[id][FOG1] = 0;
		//g_bBanned[id] = true;
	...
	if ( g_iPerfectBhop [id] [FOG2] >= MAXSEMIPERFECT )
	{
		...
		get_user_name ( id , szName , charsmax(szName) );
		get_user_authid ( id , szSteamId , charsmax(szSteamId) );
		logCheater(id, "[Anti-Cheat] Player %s(%s) [%i] is using bhop hack - MaxSemiPerfect. g_iPerfectBhop[Fog2]: %i/%i", szName, szSteamId, g_iDetections[id], g_iPerfectBhop[id][FOG2], MAXSEMIPERFECT);
	...
	if( flPlayerSpeed > 50.0 && g_iFrames[ id ] < 6 && button & IN_DUCK && ~get_user_oldbutton(id) & IN_DUCK  && pev( id, pev_flags ) & FL_ONGROUND  )
	{
		...
		if (  g_iPerfectGstrafe [ id ] [FOG1] >= MAXPERFECT || g_iPerfectGstrafe [ id ] [FOG2] >= MAXSEMIPERFECT ) {
			...
			logCheater(id, "[Anti-Cheat] Player %s(%s) [%i] is using groundstrafe hack - MaxPerfects. %i/%i %i/%i", name, steamid, g_iDetections[id], g_iPerfectGstrafe[id][FOG1], MAXPERFECT, g_iPerfectGstrafe[id][FOG2], MAXSEMIPERFECT);
		//	g_bBanned[id] = true;
	...
	if( flPlayerSpeed > 50.0 && g_iFrames[ id ] < 6 && button & IN_DUCK && ~get_user_oldbutton(id) & IN_DUCK  && pev( id, pev_flags ) & FL_ONGROUND  )
	{
		...
		if ( g_iGstrafeRatio == 60 ) {
			...
			if ( g_Result >= 95 ) {
				...
				logCheater(id, "[Anti-Cheat] Player %s(%s) [%i] is using groundstrafe hack - Ratio. %.1f", name, steamid, g_iDetections[id], g_Result);
